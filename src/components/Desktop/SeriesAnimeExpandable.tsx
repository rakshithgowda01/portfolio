...
interface SeriesAnimeExpandableProps {
  cards: SeriesAnimeCard[];
  onClose?: () => void;
}

export const SeriesAnimeExpandable = ({ cards, onClose }: SeriesAnimeExpandableProps) => {
  const [active, setActive] = useState<SeriesAnimeCard | null>(null);
  const id = useId();
  ...
  useOutsideClick(ref, () => {
    setActive(null);
    // Don't call onClose here, just close the expanded card
  });
          ...
          <div className="fixed inset-0 grid place-items-center z-[100]">
            ...
            >
              ...
              <motion.button
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute top-4 right-4 z-10 flex items-center justify-center bg-red-500 hover:bg-red-600 rounded-full h-6 w-6 transition-colors group"
                onClick={() => {
                  setActive(null);
                }}
                title="Close"
