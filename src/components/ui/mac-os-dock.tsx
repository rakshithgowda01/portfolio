...
const MacOSDock: React.FC<MacOSDockProps> = ({ 
	...
	const getResponsiveConfig = useCallback(() => {
		...
		if (smallerDimension < 480) {
			// Mobile phones (increase slightly and enable stronger hover)
			return {
	...
	// Touch handlers for mobile hover-like effect
	const handleTouchMove = useCallback((e: React.TouchEvent) => {
		...
		if (dockRef.current && e.touches.length > 0) {
			const rect = dockRef.current.getBoundingClientRect();
			const padding = Math.max(8, baseIconSize * 0.12);
			setMouseX(e.touches[0].clientX - rect.left - padding);
		}
	}, [baseIconSize]);
						...
						>
							...
							{scale > 1.1 && (
								<div 
									...
									style={{
										bottom: `${baseIconSize + 8}px`,
										left: '50%',
										transform: 'translateX(-50%)',
										opacity: Math.min(1, (scale - 1.1) / 0.3)
									}}
								>
									<div className="bg-gray-900/90 text-white text-xs px-2 py-1 rounded-md backdrop-blur-sm whitespace-nowrap shadow-lg border border-white/10">
										{app.id === 'mail' ? 'Open Contact' : app.id === 'spotify' ? 'Open Spotify' : app.name}
									</div>
